...
async function fetchAttractionData() {
    const pathSegments = window.location.pathname.split("/");
    const idNumber = pathSegments[pathSegments.length - 1];
    if (!idNumber) return;
    ...
    try {
        ...
        if (result.data) {
            ...
            renderAttraction(currentAttraction);
        }
    } catch (err) {
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    bookingBtn.addEventListener("click", async () => {
        ...
        const timeOption = document.querySelector("input[name='half-day']:checked");
        ...
        try {
            ...
            if (response.ok && result.ok) {
                ...
                alert(result.message || "預定失敗");
            }
        } catch (err) {
            console.error("建立預定時錯誤:", err);
        }
    });
});
